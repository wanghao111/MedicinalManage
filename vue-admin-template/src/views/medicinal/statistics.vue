<template>
  <div>
    <div class="container">
      ...
      <div  style="visibility:hidden;display:inline-block;width:50%;height:50%;text-align:right">
          <span>药品代码</span>
    ...
    <div class="app-container">
      ...
      >
        ...
        <el-table-column label="上月结余库存"  align="center">
          <template slot-scope="scope">
            {{ scope.row.preMonthSurplusCount }}
          </template>
        </el-table-column>
        <el-table-column label="上月结余总价"  align="center">
          <template slot-scope="scope">
            {{ scope.row.preMonthSurplusTotalPrice }}
          </template>
        </el-table-column>
        <el-table-column label="本月新增数量"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentMonthAddCount }}
          </template>
        </el-table-column>
        <el-table-column label="本月结余总价"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentMonthAddTotalPrice }}
          </template>
        </el-table-column>
        <el-table-column label="本月扣减库存"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentMonthSaleCount }}
          </template>
        </el-table-column>
        <el-table-column label="本月扣减总价"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentMonthSaleTotalPrice }}
          </template>
        </el-table-column>
        <el-table-column label="本月结余库存"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentReserve }}
          </template>
        </el-table-column>
        <el-table-column label="本月结余总价"  align="center">
          <template slot-scope="scope">
            {{ scope.row.currentTotalPrice }}
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="params.textMap[params.dialogStatus]" :visible.sync="params.dialogFormVisible">
      <el-form class="small-space" label-position="left" label-width="80px"
        ...
        <el-form-item label="药品名称" required>
            <el-input type="text;readonly:true" v-model="medicinalBuyerRecord.medicinalName" :disabled="params.dialogStatus==='update'" v-bind:readonly="params.dialogStatus==='update'">
          </el-input>
        </el-form-item>
        <el-form-item label="单价">
          <el-input type="text" v-model="medicinalBuyerRecord.unitPrice" >
          </el-input>
        </el-form-item>
...
<script>
import { pageMedicinalReserveStas} from '@/api/medicinal'
...
export default {
  ...
  methods: {
    fetchData(p) {
      ...
      pageMedicinalReserveStas(pageQuery).then(response => {
